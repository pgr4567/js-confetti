...
import { IJSConfettiConfig, IAddConfettiConfig, IPosition } from './types'
...
class JSConfetti {
    ...
    public addConfetti(confettiConfigs: IAddConfettiConfig[]): Promise<void> {
        ...
        for (const config of confettiConfigs) {
            const {
                confettiRadius,
                confettiNumber,
                confettiColors,
                emojis,
                emojiSize,
            } = normalizeConfettiConfig(config)


            for (let i = 0; i < confettiNumber / 2; i++) {
                const confetti = new ConfettiShape({
                    initialPosition: getPos(config.confettiPosition || { x: 0, y: 0.5 }),
